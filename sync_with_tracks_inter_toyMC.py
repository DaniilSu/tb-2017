...
# Usage
if len(sys.argv) < 4:
	sys.stderr.write('Usage:\t' + str(sys.argv[0]) + ' mamba_root vme_rooot (optional) numberOfRun (optional): bin_size (um) statistics_fraction (0..1.0) optional(time_window (ns)) number_of_hits_in_short_straw (optional)...\n')
	sys.exit(1)
...
if not binSizeFlag:
	...
	resolutionS = TH2F("Resolution_short", "Resolution_short", int(2.0/(0.0001*float(sys.argv[4]))), -1., 1., 1000, -1.2, 1.2)
	if enableToyMC:
		resolutionS_toy = TH2F("Resolution_short_toy", "Resolution_short_toyMC", int(2.0/(0.0001*float(sys.argv[4]))), -1., 1., 1000, -1.2, 1.2)
...
if ntrkFlag:
	nHitsInShortStraw = int(sys.argv[7])
else:
	nHitsInShortStraw = 1
...
if noiseRedFlag :
	...
	gf = TF1("gf", "gaus", -1.2, 1.2)
	gf.SetParLimits(0,5,5000)
	gf.SetParLimits(1,-1.2,1.2)
